...
const isEntryReferenceNode = node => [BLOCKS.EMBEDDED_ENTRY, INLINES.ENTRY_HYPERLINK, INLINES.EMBEDDED_ENTRY].indexOf(node.nodeType) >= 0;

const isAssetReferenceNode = node => [BLOCKS.EMBEDDED_ASSET, INLINES.ASSET_HYPERLINK].indexOf(node.nodeType) >= 0;

const isEntryReferenceField = field => _.get(field, `sys.type`) === `Entry`;

const isAssetReferenceField = field => _.get(field, `sys.type`) === `Asset`;

const getFieldProps = (contentType, fieldName) => contentType.fields.find(({
  id
}) => id === fieldName);

const getAssetWithFieldLocalesResolved = ({
  asset,
  getField
}) => {
  return { ...asset,
...
const getFieldWithLocaleResolved = ({
  ...
  if (Array.isArray(field)) {
    return field.map(fieldItem => getFieldWithLocaleResolved({
      field: fieldItem,
      contentTypesById,
      getField,
      defaultLocale,
...
const getEntryWithFieldLocalesResolved = ({
  ...
  return { ...entry,
    fields: _.mapValues(entry.fields, (field, fieldName) => {
      ...
      const fieldValue = fieldProps.localized ? getField(field) : field[defaultLocale];
...
const getNormalizedRichTextNode = ({
  ...
  if (Array.isArray(node.content)) {
    return { ...node,
      content: node.content.map(childNode => getNormalizedRichTextNode({
        node: childNode,
        contentTypesById,
        getField,
        defaultLocale
...
const getNormalizedRichTextField = ({
  ...
  if (field && field.content) {
    return { ...field,
      content: field.content.map(node => getNormalizedRichTextNode({
        node,
        contentTypesById,
        getField,
        defaultLocale
