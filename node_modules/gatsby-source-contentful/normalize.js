...
const shouldBeSkipped = (object, alreadyWalkedObjectRefs) => !object || typeof object !== `object` || alreadyWalkedObjectRefs.has(object); // Walk the object model and find any property named `sys`. If it
...
const fixIds = object => {
  ...
  while (objectsToProcess.length !== 0) {
    ...
    Object.keys(current).forEach(key => {
      ...
      if (shouldBeSkipped(currentProp, alreadyWalkedObjectRefs)) return; // The `contentful_id` is ours and we want to make sure we don't visit the
